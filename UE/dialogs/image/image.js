                ...
                switch (type) {
                    ...
                    case 'startUpload':
                        ...
                        var params = utils.serializeParam(editor.queryCommandValue('serverparam')) || '',
                            url = utils.formatUrl(actionUrl + (actionUrl.indexOf('?') == -1 ? '?' : '&') + 'encode=utf-8&' + params + "&token=" + token);
...
//  #3344 gaoaining 2019-12-27 修改token获取
            uploader.on('uploadBeforeSend', function(file, data, header) {
                //这里可以通过data对象添加POST参数
                header['X_Requested_With'] = 'XMLHttpRequest';

                header['Authorization'] =    getCookie("Admin-Token");;
                // console.log( header['Authorization'],55555);
            ...
            uploader.on('uploadSuccess', function(file, ret) {
                ...
                try {
                    ...
                    if (json.status == '200') {
                        ...
                        $file.find('.error').text(json.status).show();
                    }
                } catch (e) {
        ...
        getInsertList: function() {
            ...
            for (i = 0; i < this.imageList.length; i++) {
                ...
                data.data.forEach(element => {
                    list.push({
                        src: "/api/cms/cmsContent/downloadFile?fileId=" + element.fileId + "&token=" + token,
                        _src: "api/cms/cmsContent/downloadFile?fileId=" + element.fileId,
                        title: data.title,
                        alt: data.original,
                        floatStyle: align
                    });
                ...
                // list.push({
                //     src: "/api/cms/cmsContent/downloadFile?fileId=" + data.data.fileId + "&token=" + token,
                //     _src: "api/cms/cmsContent/downloadFile?fileId=" + data.data.fileId,
                //     title: data.title,
                //     alt: data.original,
                //     floatStyle: align
                // });
